...
\begin{frame}{Autômatos finitos --- um exemplo prático}
  \begin{columns}[T]
    \begin{column}{0.46\textwidth}
      \centering
      \vspace{10pt}
      \begin{tikzpicture}[shorten >=1pt,node distance=2.4cm,on grid,>={Stealth[]},
          every state/.style={draw=acento,thick,fill=acento!8,minimum size=8mm,font=\small}]
    ...
    \begin{column}{0.54\textwidth}
      \footnotesize
      Cada elemento de $M=(Q,\Sigma,\delta,q_0,F)$ no grafo:
      \begin{itemize}\itemsep2pt
        \item $Q=\{q_0,q_1,q_2\}$ --- os círculos;
        \item $\Sigma=\{a,b\}$ --- os rótulos das arestas;
        \item $q_0$ --- estado inicial (seta de entrada);
        \item $F=\{q_2\}$ --- estados finais (círculo duplo);
        \item $\delta$ --- as próprias arestas do grafo:
      \end{itemize}
      \vspace{-2pt}
      \centering
  ...
  \vspace{3pt}
  \begin{block}{}
    \scriptsize Reconhece as cadeias sobre $\Sigma$ que \textbf{contêm ``$ab$'' como subcadeia}.
    \textcolor{green!50!black}{\textbf{Aceita:}} \texttt{ab}, \texttt{aab}, \texttt{bab}. \quad
    \textcolor{red!70!black}{\textbf{Rejeita:}} $\varepsilon$, \texttt{ba}, \texttt{bbb}. \quad
    Reaparece adiante --- na DSL e na simulação.
  \end{block}
\end{frame}
...
\begin{frame}{Simulador}
  \begin{itemize}
    \item Executa a função de transição sobre a cadeia, registrando \textbf{todas as configurações
          intermediárias}.
    \item Exibe um \textbf{caminho único} do autômato, e não a nuvem de estados simultâneos:
          \begin{itemize}
            \item cadeia aceita $\to$ o \textbf{caminho aceitador};
            \item cadeia rejeitada $\to$ o caminho que \textbf{mais se aproxima} da aceitação.
          \end{itemize}
    \item Escolha por \textbf{busca em largura} sobre pares (estado, símbolos consumidos):
          maximiza os símbolos lidos e, no empate, minimiza a distância a um estado final.
